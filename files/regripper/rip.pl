...
# 
#-------------------------------------------------------------
if (my $plugins_file = $config{file}) {
...
# correct
	...
	my %plugins = parsePluginsFile($plugins_file);
	if (%plugins) {
	...
	foreach my $i (sort {$a <=> $b} keys %plugins) {
		eval {
			my $p_file = "$plugins{$i}.pl";
			my $p_path = catfile($plugindir, $p_file);
			require $p_path;
...
used if no other filename given is \"plugins\\plugins\"\.
  ...
  -f [plugin file]...use the plugin file (ntuser | sam | system | software | security | usrclass)
...
# choose different plugins files	
	my $pluginfile = catfile($plugindir,$file);
	if (-e $pluginfile) {
		open(FH,"<",$pluginfile);
		my $count = 1;
		while(<FH>) {
