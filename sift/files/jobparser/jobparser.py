...
class UUID:
    ...
    def __repr__(self):
        return (
            "{"
            + "{0:08X}-{1:04X}-{2:04X}-{3:04X}-{4:02X}{5:02X}{6:02X}".format(
                self.UUID0,
                self.UUID1,
                self.UUID2,
                self.UUID3,
...
class Job:
    def __init__(self, data):
        ...
        if self.UserSize > 0:
            self.User = data[self.cursor : self.cursor + self.UserSize * 2].replace(
                b"\x00", b""
            )
        ...
        if self.CommentSize > 0:
            self.Comment = data[
                self.cursor : self.cursor + self.CommentSize * 2
            ].replace(b"\x00", b"")
        ...
        self.ScheduledDate = JobDate(
            data[self.cursor : self.cursor + 20], scheduled=True
        )

    def _get_job_info(self):
        lines = []
        lines.append(
            "Product Info: {0}".format(
                products.get(self.ProductInfo, "Unknown Version")
            )
        )
        ...
        seconds = int(ms / 1000)
        lines.append(
            "Maximum Run Time: {0:02}:{1:02}:{2:02}.{3} (HH:MM:SS.MS)".format(
                hours, minutes, seconds, ms
            )
        )
        lines.append("Exit Code: {0}".format(self.ExitCode))
        lines.append(
            "Status: {0}".format(task_status.get(self.Status, "Unknown Status"))
        )
        theflags = ""
        for flag in flags:
        ...
        lines.append("Application: {0}".format(self.Name.decode()))
        if self.Parameter != "":
            lines.append("Parameters: {0}".format(self.Parameter.decode()))
        lines.append("Working Directory: {0}".format(self.WorkingDirectory))
        lines.append("User: {0}".format(self.User.decode()))
        lines.append("Comment: {0}".format(self.Comment.decode()))
    ...
    def __repr__(self):
        ...
        if self.Parameter != "":
            lines += "Parameters: {0}\n".format(self.Parameter.decode())
        lines += "Working Directory: {0}\n".format(self.WorkingDirectory)
        lines += "User: {0}\n".format(self.User.decode())
        lines += "Comment: {0}\n".format(self.Comment.decode())
