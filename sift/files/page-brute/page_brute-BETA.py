            ...
            for yara_file in glob.glob(os.path.join(RULES, "*.yar")):
                try:
                    ...
                    try:
                        ...
                        print(
                            "..... SKIPPING: Could not open file for reading: %s "
                            % yara_file
                        )
                except:
    ...
    if args.rules:
        ...
        try:
            ...
            if os.path.isfile(RULES):
                RULETYPE = "FILE"
                print(
                    "[+] - YARA rule of File type provided for compilation: %s" % RULES
                )
            elif os.path.isdir(RULES):
                print(
                    "[+] - YARA rule of Folder type provided for compilation: %s"
                    % RULES
                )
                RULETYPE = "FOLDER"
        except:
            print(
                "[!] - Possible catastrophic error with the provided rule file...exiting."
            )
            sys.exit()
    else:
        try:
            ...
            print(
                '[!] - Could not locate "default_signature.yar". Find it or provide custom signatures via --rules. Exiting.'
            )
            sys.exit()
            ...
            if not is_block_null(raw_page):
                ...
                if INVERT == True:
                    if matched == False:
                        ...
                        print(
                            "        [!] BLOCK DOES NOT MATCH ANY KNOWN SIGNATURE "
                            + str(page_id)
                        )
                        if not os.path.exists(CHUNK_OUTPUT_DIR):
                            os.makedirs(CHUNK_OUTPUT_DIR)
