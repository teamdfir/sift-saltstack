...
GetOptions(\%config, qw(dir|d=s file|f=s exe|e path|p server|s=s info|i tln|t csv|c help|?|h));
...
foreach my $file (@files) {
	...
	foreach my $path (@paths) {
		$pref{$file}{exepath} = $path if ($path =~ m/$e$/i);	
	}
	$pref{$file}{exepath} = $name if ($pref{$file}{exepath} eq "");
