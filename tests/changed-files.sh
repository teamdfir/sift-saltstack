...
echo "States To Test:"
echo "`git diff-tree --no-commit-id --name-only -r ${TRAVIS_COMMIT} | grep "sift/" | grep -v "init.sls"`"

for FILE in `git diff-tree --no-commit-id --name-only -r ${TRAVIS_COMMIT} | grep "sift/" | grep -v "init.sls"`; do
